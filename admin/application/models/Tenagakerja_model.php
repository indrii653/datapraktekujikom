...
class Tenagakerja_model extends CI_Model
{
     ...
     */
    function tenagaListingCount($searchText = '')
    {
        $this->db->select('*');
        $this->db->from('tenagakerja as BaseTbl');
        if(!empty($searchText)) {
     ...
     */
    function tenagaListing($searchText = '', $page, $segment)
    {
        $this->db->select('*');
        $this->db->from('tenagakerja as BaseTbl');
        if(!empty($searchText)) {
     ...
     */
    function addNewtenaga($tenagaInfo)
    {
        $this->db->trans_start();
        $this->db->insert('tenagakerja', $tenagaInfo);
     ...
     */
    function gettenagaInfo($id)
    {
        $this->db->select('*');
        $this->db->from('tenagakerja');
        $this->db->where('id', $id);
     ...
     * @param number $taskId : This is task id
     */
    function edittenaga($tk, $id)
    {
        $this->db->where('id', $id);
        $this->db->update('tenagakerja', $tk);
        
        return TRUE;
    }

    function deletetenaga( $id)
    {
        $this->db->where('id', $id);
        $this->db->delete('tenagakerja');

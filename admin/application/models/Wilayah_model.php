...
class Wilayah_model extends CI_Model
{
     ...
     */
    function wilayahListingCount($searchText = '')
    {
        $this->db->select('*');
        $this->db->from('wilayah as BaseTbl');
        if(!empty($searchText)) {
     ...
     */
    function wilayahListing($searchText = '', $page, $segment)
    {
        $this->db->select('*');
        $this->db->from('wilayah as BaseTbl');
        if(!empty($searchText)) {
     ...
     */
    function addNewwilayah($wilayahInfo)
    {
        $this->db->trans_start();
        $this->db->insert('wilayah', $wilayahInfo);
     ...
     * @param number $taskId : This is task id
     */
    function editwilayah($w, $id)
    {
        $this->db->where('id', $id);
        $this->db->update('wilayah', $w);
        
        return TRUE;
    }
    function deletewilayah($w, $id)
    {
        $this->db->where('id', $w);
        $this->db->delete('wilayah', $w);
